...
while True:
    ...
    data_packet = pk.construct_data_packet(speed,fuel_level,temperature)
    print("Sending Data Packet:", data_packet.hex())
	
    temperature_int = int(temperature * 100)
    fl_int = int(fuel_level)
    command_packet = struct.pack('>BBHHBHB', START_BYTE, DATA_PACKET, speed, fl_int, temperature_int,0)
	# Add checksum
